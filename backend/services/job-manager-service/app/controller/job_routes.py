...
def add_application():
    ...
    required_field= ["jobId","applicationId", "applicantEmail", "status","resumeUrl"]
    logger.info("✅ Inside add_application() method....... ",job_application_info)
    if not all(field in job_application_info for field in required_field):
        logger.warning(f"❌ Missing required job fields: {job_application_info}")
        return missing_fields_exception()

    job_id = job_application_info["jobId"]
    application = {
        "applicationId" : job_application_info["applicationId"],
        "applicantEmail" : job_application_info["applicantEmail"],
        "status":job_application_info["status"],
        "resumeUrl":job_application_info["resumeUrl"]
